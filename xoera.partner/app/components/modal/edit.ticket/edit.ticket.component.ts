...
@Component({
    ...
    selector: 'xoera-partner-modal-edit-ticket',
})

export class XoeraPartnerModalEditTicketComponent implements OnInit, OnDestroy {
    dialog: DialogData;
    item: TicketEntity;
    ...
    ngOnInit() {
        if (this.eventDialog == null) {
            this.eventDialog = this.dialogService.EventDialog.subscribe((item: DialogData) => {
                if (item.type == <number>DialogType.EditTicket) {
                    this.dialog = item;
                    this.visible = true;
    ...
    public async confirmModal() {
        if (this.dialog) {
            let valid = await validation(this.item, this.event, this.service, ['Title', 'Content']);
            if (valid) {
